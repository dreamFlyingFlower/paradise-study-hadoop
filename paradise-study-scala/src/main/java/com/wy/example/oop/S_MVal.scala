...
object S_MVal {
  val amt = CurrencyScala(1000.0, "EUR")

  // 中置表达式
  amt match { case a CurrencyScala u => a + " " + u }
  ...
  List(1, 7, 2, 9) match {
    case first :: second :: rest =>
      println(first); println(second); println(rest)
    case _ => 0
  }
  ...
  List(List(1, 7), List(2, 9)) match {
    case (first :: second) :: rest1 =>
      println(first); println(second); println(rest1)
    case _ => 0
  }

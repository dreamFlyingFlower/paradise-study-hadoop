...
import com.wy.common.ObjectMetaData;
...
/**
 * @apiNote 操作Bucket和Object的service类
 * @author ParadiseWY
 * @date 2020年2月10日 上午12:13:54
 */
@Slf4j
...
public class HOSStoreServiceImpl implements HOSStoreService {
	...
	public void createBucketStore(String bucket) throws Exception {
		// 创建目录表
		HBaseServiceImpl.createTable(this.connection, HOSConst.getDirTableName(bucket), HOSConst.getDirColumnFamilies(),
				null);
		// 创建文件表
		HBaseServiceImpl.createTable(this.connection, HOSConst.getObjTableName(bucket), HOSConst.getObjColumnFamilies(),
				HOSConst.OBJ_REGIONS);
		// 将其添加到seq表
		Put put = new Put(bucket.getBytes());
		put.addColumn(HOSConst.BUCKET_DIR_SEQ_CF_BYTES, HOSConst.BUCKET_DIR_SEQ_COLUMN_QUALIFIER_BYTES,
				Bytes.toBytes(0L));
	...
	public void createSeqTable() throws Exception {
		HBaseServiceImpl.createTable(this.connection, HOSConst.BUCKET_DIR_SEQ_TABLE,
				new String[] { HOSConst.BUCKET_DIR_SEQ_CF }, null);
	}
	...
	public void put(String bucket, String key, ByteBuffer content, long length, String mediaType,
		...
		lock = new InterProcessMutex(zookeeperClient,
				File.separator + "hos" + File.separator + bucket + File.separator + lockey);
		lock.acquire();
		// 上传文件
		String fileKey = hash + "_" + key.substring(key.lastIndexOf(File.separator) + 1);
		Put contentPut = new Put(fileKey.getBytes());
		if (!StringUtils.isEmpty(mediaType)) {
			contentPut.addColumn(HOSConst.OBJ_META_CF_BYTES, HOSConst.OBJ_MEDIATYPE_QUALIFIER, mediaType.getBytes());
		}
		// TODO 添加文件信息
		// 判断文件大小
		if (length <= HOSConst.FILE_STORE_THRESHOLD) {
			...
			contentPut.addColumn(HOSConst.OBJ_CONTENT_CF_BYTES, byteBuffer, System.currentTimeMillis(), content);
			byteBuffer.clear();
		} else {
			// 放入HDFS
	...
	public List<HOSObjectSummary> getSummaries(String bucket, String key, String startKey, String endKey)
		...
		if (seqId != null && name1.length() > 0) {
			...
			ResultScanner scanner1 = HBaseServiceImpl.getScanner(connection, HOSConst.getObjTableName(bucket), start,
					tail);
			Result result = null;
		...
		ResultScanner scanner2 = HBaseServiceImpl.getScanner(connection, HOSConst.getDirTableName(bucket), startKey,
				endKey);
		Result result = null;
		while ((result = scanner2.next()) != null) {
			String seqId2 = Bytes.toString(result.getValue(HOSConst.DIR_META_CF_BYTES, HOSConst.DIR_SEQID_QUALIFIER));
			if (seqId2 == null) {
				continue;
			}
	...
	public List<HOSObjectSummary> list(String bucket, String startKey, String endKey) throws Exception {
		...
		while ((result = scanner2.next()) != null) {
			String seqId2 = Bytes
					.toString(result.getValue(HOSConst.DIR_META_CF_BYTES, HOSConst.DIR_SEQID_QUALIFIER));
			if (seqId2 == null) {
				continue;
			}
			String dir = Bytes.toString(result.getRow());
	...
	public ObjectListResult listDir(String bucket, String dir, String startKey, int maxCount) throws Exception {
		...
		get.addFamily(HOSConst.DIR_SUB_CF_BYTES);
		if (!StringUtils.isEmpty(startKey)) {
			get.setFilter(new QualifierFilter(CompareOperator.GREATER_OR_EQUAL,
		...
		objScan.addFamily(HOSConst.OBJ_META_CF_BYTES);
		...
		HOSObjectSummary nextMarkerObj = objectSummaryList.size() > maxCount
				? objectSummaryList.get(objectSummaryList.size() - 1)
				: null;
	...
	@Override
	public ObjectListResult listDirByPrefix(String bucket, String dir, String startKey, String keyPrefix, int maxCount)
			throws Exception {
		if (startKey == null) {
			startKey = "";
		}
		...
		if (startKey.length() > 0) {
			filterList.addFilter(new QualifierFilter(CompareOperator.GREATER_OR_EQUAL,
					new BinaryComparator(Bytes.toBytes(startKey))));
		}
		int maxCount1 = maxCount + 2;
		Result dirResult = HBaseServiceImpl.getRow(connection, HOSConst.getDirTableName(bucket), dir, filterList);
		List<HOSObjectSummary> subDirs = null;
		if (!dirResult.isEmpty()) {
			subDirs = new ArrayList<>();
			for (Cell cell : dirResult.rawCells()) {
		...
		ResultScanner objScanner = HBaseServiceImpl.getScanner(connection, HOSConst.getObjTableName(bucket), objScan);
		List<HOSObjectSummary> objectSummaryList = new ArrayList<>();
		Result result = null;
	...
	public HOSObject getObject(String bucket, String key) throws Exception {
		...
		HOSObject hosObject = new HOSObject();
		ObjectMetaData objectMetaData = new ObjectMetaData();
		long length = Bytes.toLong(result.getValue(HOSConst.OBJ_META_CF_BYTES, HOSConst.OBJ_LENGTH_QUALIFIER));
		objectMetaData.setLength(length);
		objectMetaData.setBucket(bucket);
		objectMetaData.setKey(key);
		objectMetaData.setLastModifyTime(result.rawCells()[0].getTimestamp());
		objectMetaData.setMediaType(
				Bytes.toString(result.getValue(HOSConst.OBJ_META_CF_BYTES, HOSConst.OBJ_MEDIATYPE_QUALIFIER)));
		byte[] attrs = result.getValue(HOSConst.OBJ_META_CF_BYTES, HOSConst.OBJ_PROPERTY_QUALIFIER);
		if (attrs != null) {
			objectMetaData.setAttrs(JsonUtil.fromJson(Map.class, Bytes.toString(attrs)));
		}
		hosObject.setMetaData(objectMetaData);
		// 读取文件内容
		if (result.containsNonEmptyColumn(HOSConst.OBJ_CONTENT_CF_BYTES, HOSConst.OBJ_CONTENT_QUALIFIER)) {
			// 从HBase中读取
			ByteArrayInputStream inputStream = new ByteArrayInputStream(
					result.getValue(HOSConst.OBJ_CONTENT_CF_BYTES, HOSConst.OBJ_CONTENT_QUALIFIER));
			hosObject.setContent(inputStream);
		} else {
			// 从HDFS中读取
			String fileDir = HOSConst.FILE_STORE_ROOT + File.separator + bucket + File.separator + sequenceId;
			InputStream inputStream = this.hdfsService.openFile(fileDir,
					key.substring(key.lastIndexOf(File.separator)));
			hosObject.setContent(inputStream);
		}
		return hosObject;
	}
	...
	public void deleteObject(String bucket, String key) throws Exception {
		...
		if (key.endsWith(File.separator)) {
			...
			if (!isDirEmpty(bucket, key)) {
				throw new ResultException(ErrorCode.ERROR_PERMISSION_DENIED, "dir is not empty");
			}
			// 获取锁
			...
			if (name.length() > 0) {
				...
				HBaseServiceImpl.deleteColumnQualifier(connection, HOSConst.getDirTableName(bucket), parentDir,
						HOSConst.DIR_SUB_CF, name);
			}
			// 从目录表中删除
			...
			// 查询长度
			get.addColumn(HOSConst.OBJ_META_CF_BYTES, HOSConst.OBJ_LENGTH_QUALIFIER);
	...
	private String getDirSequenceId(String bucket, String key) throws Exception {
		...
		return Bytes.toString(result.getValue(HOSConst.DIR_META_CF_BYTES, HOSConst.DIR_SEQID_QUALIFIER));
	}
	...
	private String putDir(String bucket, String key) throws Exception {
		...
		try {
			...
			if (name.length() > 0) {
				...
				put.addColumn(HOSConst.DIR_SUB_CF_BYTES, Bytes.toBytes(name), Bytes.toBytes(1));
			...
			dirPut.addColumn(HOSConst.DIR_META_CF_BYTES, HOSConst.DIR_SEQID_QUALIFIER, Bytes.toBytes(hash));
	...
	private String makeDirSequenceId(String bucket) throws Exception {
		long incrementColumnValue = HBaseServiceImpl.incrementColumnValue(connection, HOSConst.BUCKET_DIR_SEQ_TABLE,
				bucket, HOSConst.BUCKET_DIR_SEQ_CF_BYTES, HOSConst.BUCKET_DIR_SEQ_COLUMN_QUALIFIER_BYTES, 1);
	...
	private void getDirAllFiles(String bucket, String dir, String seqId, List<HOSObjectSummary> keys, String endKey)
			throws IOException {
		byte[] max = Bytes.createMaxByteArray(100);
		byte[] tail = Bytes.add(Bytes.toBytes(seqId), max);
		...
		Scan scan = new Scan().withStartRow(Bytes.toBytes(seqId)).withStopRow(tail);
		scan.setFilter(HOSConst.OBJ_META_SCAN_FILTER);
		ResultScanner scanner = HBaseServiceImpl.getScanner(connection, HOSConst.getObjTableName(bucket), scan);
		Result result = null;

...
public class HosClientServiceImpl implements HosClientService {
	...
	public HosClientServiceImpl(String endPoints, String token) {
		...
		OkHttpClient.Builder httpClientBuilder = new OkHttpClient.Builder().connectTimeout(15, TimeUnit.SECONDS)
				.readTimeout(120L, TimeUnit.SECONDS).writeTimeout(120L, TimeUnit.SECONDS).retryOnConnectionFailure(true)
				.connectionPool(pool);
	...
	public void createBucket(String bucketName, String detail) throws Exception {
		...
		Request request = new Request.Builder().headers(headers)
				.url(new HttpUrl.Builder().scheme(schema).host(host).port(port).addPathSegment("/hos/v1/bucket")
						.addQueryParameter("bucket", bucketName).addQueryParameter("detail", detail).build())
	...
	public void deleteBucket(String bucketName) throws Exception {
		...
		Request request = new Request.Builder()
				.headers(headers).url(new HttpUrl.Builder().scheme(schema).host(host).port(port)
						.addPathSegment("/hos/v1/bucket").addQueryParameter("bucket", bucketName).build())
	...
	public List<Bucket> listBucket() throws Exception {
		Headers headers = this.buildHeaders(null, token, null);
		Request request = new Request.Builder().headers(headers).url(new HttpUrl.Builder().scheme(schema).host(host)
				.port(port).addPathSegment("/hos/v1/bucket/list").build()).get().build();
		...
		List<Bucket> bucketModels = JsonUtil.fromJsonList(Bucket.class, json);
		return bucketModels;
	}
	...
	public void putObject(PutRequest putRequest) throws Exception {
		...
		Request request = new Request.Builder().headers(headers).url(
				new HttpUrl.Builder().scheme(schema).host(host).port(port).addPathSegment("/hos/v1/object").build())
				.post(requestBody).build();
	...
	@Override
	public void putObject(String bucket, String key, byte[] content, String mediaType, String contentEncoding)
			throws Exception {
		PutRequest putRequest = new PutRequest(bucket, key, content, mediaType);
	...
	@Override
	public void putObject(String bucket, String key, File content, String mediaType) throws Exception {
		if (!content.exists()) {
	...
	@Override
	public void putObject(String bucket, String key, File content, String mediaType, String contentEncoding)
			throws Exception {
		PutRequest putRequest = new PutRequest(bucket, key, content, mediaType);
		putRequest.setContentEncoding(contentEncoding);
		this.putObject(putRequest);
	...
	public void deleteObject(String bucket, String key) throws Exception {
		...
		Request request = new Request.Builder().headers(headers)
				.url(new HttpUrl.Builder().scheme(schema).host(host).port(port).addPathSegment("/hos/v1/object")
						.addQueryParameter("bucket", bucket).addQueryParameter("key", key).build())
				.delete(requestBody).build();
	...
	public HOSObjectSummary getObjectSummary(String bucket, String key) throws Exception {
		...
		Request request = new Request.Builder().headers(headers)
				.url(new HttpUrl.Builder().scheme(this.schema).host(this.host).port(this.port)
						.addPathSegment("/hos/v1/object/info").addQueryParameter("bucket", bucket)
	...
	public ObjectListResult listObject(String bucket, String startKey, String endKey) throws IOException {
		...
		Request request = new Request.Builder().headers(headers)
				.url(new HttpUrl.Builder().scheme(this.schema).host(this.host).port(this.port)
						.addPathSegment("/hos/v1/object/list").addQueryParameter("bucket", bucket)
						.addQueryParameter("startKey", startKey).addQueryParameter("endKey", endKey).build())
				.build();
		Response response = client.newCall(request).execute();
		if (response.isSuccessful()) {
			String json = response.body().string();
	...
	@Override
	public ObjectListResult listObjectByPrefix(String bucket, String dir, String prefix, String startKey)
			throws IOException {
		Headers headers = this.buildHeaders(null, this.token, null);
		Request request = new Request.Builder().headers(headers)
				.url(new HttpUrl.Builder().scheme(this.schema).host(this.host).port(this.port)
						.addPathSegment("/hos/v1/object/list/prefix").addQueryParameter("bucket", bucket)
						.addQueryParameter("prefix", prefix).addQueryParameter("dir", dir)
						.addQueryParameter("startKey", startKey).build())
				.build();
		Response response = client.newCall(request).execute();
		if (response.isSuccessful()) {
			String json = response.body().string();
	...
	public ObjectListResult listObjectByDir(String bucket, String dir, String startKey) throws IOException {
		...
		Request request = new Request.Builder().headers(headers)
				.url(new HttpUrl.Builder().scheme(this.schema).host(this.host).port(this.port)
						.addPathSegment("/hos/v1/object/list/dir").addQueryParameter("bucket", bucket)
						.addQueryParameter("dir", dir).addQueryParameter("startKey", startKey).build())
				.build();
		Response response = client.newCall(request).execute();
		if (response.isSuccessful()) {
			String json = response.body().string();
	...
	public HOSObject getObject(String bucket, String key) throws IOException {
		...
		Request request = new Request.Builder().headers(headers)
				.url(new HttpUrl.Builder().scheme(this.schema).host(this.host).port(this.port)
						.addPathSegments("/hos/v1/object/content").addQueryParameter("bucket", bucket)
						.addQueryParameter("key", key).build())
				.get().build();
		Response response = client.newCall(request).execute();
		if (response.isSuccessful()) {
	...
	public Bucket getBucketInfo(String bucketName) throws IOException {
		...
		String json = response.body().string();
		return JsonUtil.fromJson(Bucket.class, json);
	}
		...
		if (attrs != null && attrs.size() > 0) {
			attrs.forEach(new BiConsumer<String, String>() {
				...
				public void accept(String s, String s2) {
					headerMap.put(HOSConst.COMMON_ATTR_PREFIX + s, s2);
				}
			});
		}
	...
	private ObjectMetaData buildMetaData(Response response) {
		...
		metaData.setBucket(response.header(HOSConst.COMMON_OBJ_BUCKET));
		metaData.setKey(response.header(HOSConst.COMMON_OBJ_KEY));
		...
		metaData.setLength(Long.parseLong(response.header(HOSConst.RESPONSE_OBJ_LENGTH)));
		return metaData;
	}
}